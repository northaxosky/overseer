...
fn inspect(binary: &BinaryScan, expected: Generation) -> Option<Finding> {
    if !binary.present {
        return Some(Finding::new(
            Severity::Warning,
            format!("`{}` is missing from the game folder", binary.name),
            Some("Verify the game files through your storefront (or reinstall) so every core binary is \
            present and matches your game version.".to_owned())
        ));
    }
    ...
    match binary.edition {
        ...
        None => Some(Finding::new(
            Severity::Warning,
            format!("could not verify `{}` (unrecognized version)", binary.name),
            Some(
